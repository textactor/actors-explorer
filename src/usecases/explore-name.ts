...
export class ExploreName extends UseCase<
  string | string[],
  Actor | null,
  void
> {
  ...
  constructor(
    ...
    this.exploreWikiEntities = new ExploreWikiEntitiesByNames(
      locale,
      this.entityRep,
  ...
  protected async innerExecute(name: string | string[]): Promise<Actor | null> {
    ...
    const actor = await this.actorBuilder.execute(
      ActorNameCollection.fromArray(name, lang)
    );
