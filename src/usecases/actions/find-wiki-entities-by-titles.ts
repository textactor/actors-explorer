...
export class FindWikiEntitiesByTitles extends UseCase<
  string[],
  WikiEntity[],
  null
> {
  ...
  protected async innerExecute(titles: string[]): Promise<WikiEntity[]> {
    ...
    if (wikiEntities.length === 0) {
      ...
      debug(
        `Found wiki entities for ${titles.join("|")}: ${wikiEntities.map(
          (item) => item.label
        )}`
      );
    }

    let entities = wikiEntities.map((wikiEntity) =>
      this.wikiEntityBuilder.build({ wikiEntity })
    );

    entities = entities.filter(
      (item) => !WikiEntityHelper.isDisambiguation(item)
    );
