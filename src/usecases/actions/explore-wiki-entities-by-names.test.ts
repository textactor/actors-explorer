...
test("Moscow multi names", async (t) => {
  ...
  const moscowSearch = await searchRepository.getById(
    WikiSearchNameHelper.createId("Москва", locale.lang, locale.country)
  );
  t.truthy(moscowSearch);

  const moscowTitle = await titleRepository.getById(
    WikiTitleHelper.createId("Москва", locale.lang)
  );
  t.truthy(moscowTitle);

  const moscowEntity = await entityRepository.getByNameHash(
    WikiEntityHelper.nameHash("Москва", locale.lang)
  );
