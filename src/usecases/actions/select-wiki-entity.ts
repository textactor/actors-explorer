...
export class SelectWikiEntity extends UseCase<
  string[],
  WikiEntity | null,
  void
> {
  ...
  protected async innerExecute(names: string[]): Promise<WikiEntity | null> {
    ...
    if (
      entities.length === 0 ||
      countryWikiEntities(entities, this.locale.country).length === 0
    ) {
      let entitiesByPartialNames: WikiEntity[] = [];
      for (let nameHash of nameHashes) {
        const list = await this.wikiEntityRepository.getByPartialNameHash(
          nameHash
        );
        entitiesByPartialNames = entitiesByPartialNames.concat(list);
      }

      entitiesByPartialNames = countryWikiEntities(
        entitiesByPartialNames,
        this.locale.country
      );
      if (entitiesByPartialNames.length) {
        debug(
          `found locale entities by partial name: ${entitiesByPartialNames.map(
            (item) => item.name
          )}`
        );
      ...
      if (entities.length === 0) {
        debug(
          `NOT Found wikientity by partial names: ${JSON.stringify(names)}`
        );
        return null;
      }
    }
...
export function sortWikiEntities(
  ...
  if (countryEntities.length) {
    ...
    const countryEntityPopularity = WikiEntityHelper.getPopularity(
      countryEntities[0].rank
    );

    useCountryEntity =
      topEntityPopularity < EntityPopularity.POPULAR ||
      countryEntityPopularity > EntityPopularity.LOW;
    ...
    if (useCountryEntity && isPriorityEntity(topEntity)) {
      ...
      debug(
        `using POPULAR entity: ${topEntity.name}(${topEntity.rank}-${topEntityPopularity})>${countryEntities[0].rank}-(${countryEntityPopularity})`
      );
    }
...
function countryWikiEntities(
  ...
  return entities.filter(
    (item) => item.countryCodes && item.countryCodes.includes(country)
  );
}
