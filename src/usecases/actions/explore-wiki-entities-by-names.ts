...
export class ExploreWikiEntitiesByNames extends UseCase<
  string[],
  string[],
  void
> {
  ...
  constructor(
    ...
    this.createOrUpdateWikiSearchName = new CreateOrUpdateWikiSearchName(
      wikiSearchNameRep
    );

    this.exploreWikiEntitiesByTitles = new FindWikiEntitiesByTitles(
      locale,
      knownNames
    );
  ...
  protected async innerExecute(names: string[]): Promise<string[]> {
    ...
    for (let name of names) {
      const searchName = await this.wikiSearchNameRep.getById(
        WikiSearchNameHelper.createId(name, lang, country)
      );
      if (
        searchName &&
        searchName.updatedAt &&
        searchName.updatedAt * 1000 > Date.now() - ms("7days")
      ) {
      ...
      await this.createOrUpdateWikiSearchName.execute(
        WikiSearchNameHelper.build({
          name,
          lang,
          country
        })
    ...
    for (let title of initalTitles) {
      const wikiTitle = await this.wikiTitleRep.getById(
        WikiTitleHelper.createId(title, lang)
      );
      if (
        wikiTitle &&
        wikiTitle.updatedAt &&
        wikiTitle.updatedAt * 1000 > Date.now() - ms("10days")
      ) {
    ...
    for (let title of titles) {
      await this.createOrUpdateWikiTitle.execute(
        WikiTitleHelper.build({ title, lang })
      );
    }
