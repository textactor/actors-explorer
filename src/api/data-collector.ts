...
export function createDataContainerApi(
  ...
  return {
    async createCollector(data: NewContainerParams): Promise<DataCollector> {
      const container = await containerRep.create(
        ConceptContainerHelper.build(data)
      );
      ...
      return {
        ...
        async pushText(text: string): Promise<void> {
          if (container.status === ConceptContainerStatus.NEW) {
            ...
            await containerRep.update({
              id: container.id,
              set: { status: ConceptContainerStatus.COLLECTING }
            });
        ...
        async end(): Promise<void> {
          ...
          await containerRep.update({
            id: container.id,
            set: { status: ConceptContainerStatus.COLLECT_DONE }
          });
        }
      };
    },
    findConceptContainer(
      data: FindContainerParams
